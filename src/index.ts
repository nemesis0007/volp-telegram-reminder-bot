...
function page(content: string) {
  ...
  *{box-sizing:border-box}body{min-height:100vh;margin:0;color:var(--ink);font:16px/1.5 Roboto,Arial,sans-serif}.shell{min-height:100vh;display:grid;grid-template-columns:1fr 1fr;background:#fff}
  ...
  .eye-toggle{width:29px;height:33px;display:grid;place-items:center;margin:0;border:0;background:transparent;color:#7d7d7d;cursor:pointer}.eye-shape{position:relative;width:18px;height:12px;border:2px solid currentColor;border-radius:75% 8% 75% 8%;transform:rotate(45deg)}.eye-shape:after{position:absolute;top:50%;left:50%;width:5px;height:5px;border-radius:50%;background:currentColor;content:"";transform:translate(-50%,-50%)}.eye-toggle:focus-visible{outline:3px solid rgba(236,169,24,.45);outline-offset:1px}
...
async function connectGet(env: Env, token: string) {
  ...
  return html(page(`<a id="open-browser" class="browser-link" href="#" target="_blank" rel="noopener noreferrer" hidden>Open in browser ↗</a><p class="signin-title">SIGN <span>IN</span></p>
    <form id="connect-form">
      ...
      <div class="field-row"><span class="field-icon lock-icon" aria-hidden="true"></span><label for="password"><span class="sr-only">VOLP password</span><input id="password" type="password" name="password" autocomplete="current-password" placeholder="VOLP password" required maxlength="300"></label><button class="eye-toggle" type="button" aria-label="Show password" aria-pressed="false"><span class="eye-shape" aria-hidden="true"></span></button></div>
